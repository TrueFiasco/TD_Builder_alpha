...
| `expertise/` | Hand-curated YAML "working memory" (operator patterns, GLSL/Python templates, build rules, palette catalogs). | read by the builder (`td_build_project`) for conversion/build rules |
...
| `Stratergies/` | Notes on rules/enforcement for using the experts well (with and without orchestration). | humans + prompt authors |
