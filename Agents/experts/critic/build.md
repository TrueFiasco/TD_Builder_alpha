...
Before approving a network design for building, validate with the `td_validate` MCP tool (runs the 7-stage pipeline):
...
2. Validate with the `td_validate` MCP tool (runs the 7-stage pipeline) on the design's network JSON
