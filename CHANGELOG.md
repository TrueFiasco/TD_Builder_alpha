...
- **T0 Tag Polish — response formatting + four lying strings** (remediation map ticket 25,
  ...
  (Schema → Semantic → Grounding → Reference → Component wiring → Logical → TD Rules), along
  with the same stale count in `MCP/engine/validation/pipeline.py`'s module docstring.
