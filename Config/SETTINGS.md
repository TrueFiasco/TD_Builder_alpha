...
- `TD_BUILDER_TRUST_KB=1` — **skip pickle integrity verification** for the KB (bm25 index + knowledge
  ...
  the pinned hashes in `scripts/vector_db_release.json` — defense-in-depth against a KB corrupted in
  transit (bad download / poisoned build cache), not against a local writer of `KB/`. Set this only
  while actively rebuilding KB internals; the durable fix for your own KB is
