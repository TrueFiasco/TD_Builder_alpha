...
All the input attributes are available for reading. For attributes from a different class than the selected class, the functions to access the attributes have a prefix with the attribute class, similarly to the GLSL Advanced POP. For example if working on point attributes, an attribute called "Attrib" can be accessed with TDIn_Attrib() if it's a point attribute, TDInVert_Attrib() if it's a vertex attribute, and TDInPrim_Attrib() if it's a primitive attribute. See below for details.
...
If the number of elements for the selected class and input is known on the GPU and not on the CPU (see POP#Indirect_POPs_and_Memory_Management), an indirect command buffer is filled, and the compute shader is launched with an indirect dispatch.
...
Here's a code snippet showing how to iterate on all the points of a primitive:

```glsl
for (int i = 0; i < TDInputNumVertsPerPrim(); i++)
{
	uint vertId = TDInputPrimVertsStartIndex() + i;
	uint pointId = TDInputPointIndex(0, vertId);
}
```

When the number of output points is changed, references to unchanged input point attributes are not created anymore, since there would be a mismatch between the number of points in the existing attribute buffers and the new number of points. It is left to the user to manually update the attributes, for existing and new points. Similarly, when vertices or primitives numbers are changed, references to unchanged input vertices and primitives attributes are not created anymore.
...
##### Input Attributes From Non Selected Attribute Class

- If the attribute class is Point , Vertex and Primitive attributes can be accessed with TDInVert_AttribName() and TDInPrim_AttribName().
- If the attribute class is Vertex , Point and Primitive attributes can be accessed with TDInPoint_AttribName() and TDInPrim_AttribName().
- If the attribute class is Primitive , Point and Vertex attributes can be accessed with TDInPoint_AttribName() and TDInVert_AttribName().
...
//output variable for point, prim, vert attributes
```
...
One thread per output vert in current primitive batch
...
One thread per output prim in current primitive batch
...
Since the GLSL Copy POP doesn't have other inputs, other POP attributes can be accessed with the TDBuffer() function and related constants, see Write_GLSL_POPs#GLSL_TOP_and_MAT:_POP_attribute_access
...
```glsl
const uint cTDDimSizeN; //constant that holds the number of dimensions for input N
uint[cTDDimSizeN] TDDimensionN(); //returns an array with the dimensions for input N
uint[cTDDimSizeN] TDDimCoordsN(uint pointIndex); //returns an array with the coordinates of pointIndex relative to dimensions for input N: matches _DimI[] built-in attribute
uint TDDimPointIndexN(uint[cTDDimSizeN] coords); //returns the point index at coordinates coords for input N
```

For convenience, if N is not included, the constant and functions refer to the first input

```glsl
const uint cTDDimSize;
uint[cTDDimSize] TDDimension();
uint[cTDDimSize] TDDimCoords(uint pointIndex);
uint TDDimPointIndex(uint[cTDDimSize] coords);
```

The functions are also available for the inputs of the extra outputs in the GLSL Advanced POP:
...
For line strips this doesn't include the restart index at the end of the line strip
```
...
Common Functions on the Write a GLSL material page Write_a_GLSL_Material#Common_Functions
...
# TD Builder Alpha — Addendum (not part of the official wiki)

The notes below are findings from live testing on top of the TD wiki text above.
They are NOT part of the official Derivative documentation. Trust the wiki text
above for anything that contradicts; trust these for the specific traps they call
out (each is verified by a live test session that produced a real reproducer).

## Uniform declaration: GLSL POP AUTO-declares Arrays-page uniforms as float[]

On the GLSL POP, scalar uniforms assigned via the Arrays page are auto-injected
into the shader namespace **as a `float[]` array**, not a bare `float`. Two
implications:

1. **Do NOT redeclare in source.** `uniform float uLfo;` in your shader will
   collide with the auto-declared `uLfo` and the compile will fail with
   "redefinition" or "already defined."
2. **Access via index 0.** Use `uLfo[0]`, not `uLfo`. The auto-declared form is
   an array even when you set `array0type=float` and `array0arraytype=single`.

This is the OPPOSITE of the GLSL TOP convention (see
Write_a_GLSL_TOP.md addendum), where you MUST declare manually. Mixing the two
in your head produces a "compiles but reads as 0" symptom on the TOP and a
"redefinition error" on the POP. They're symmetric inverse traps.

## Output-attribute declaration order matters

If your shader writes to an attribute that is not the GLSL POP's first input
(i.e. a NEW attribute, not pass-through), you must declare it on the
**Create-Attributes** page (`attr0name='custom'`, `attr0customname='myattr'`,
`attr0type='float'`, etc.) before referencing it in the shader. Listing it only
in `outputattrs` is insufficient — the shader will fail to compile with
"undeclared identifier."

Order of operations in build time:
1. Set `numthreadsmode`, `attrclass`, `outputaccess`, `initoutputattrs`.
2. Set Create-Attributes for any new output attribute.
3. Set the `computedat` parameter (the shader Text DAT).
4. Set uniforms (Vectors / Arrays / Samplers pages).
5. Cook.
