...
To write to the output, use the GLSL function TDImageStoreOutput()
...
## Uniform binding: the Constants page DOES NOT bind GLSL TOP uniforms

The GLSL TOP exposes a Constants page in its parameter UI, but **assigning a
uniform there does not bind it**. The shader will compile clean ("Compiled
Successfully" in the info DAT) and the uniform will silently read as 0. This
is a true silent failure — the only signal is `op('<glsltop>').warnings()`,
which reports:

```
Warning: Uniform 'X' is not assigned. Please assign it on the Colors or Vectors page.
```
...
## Uniform declaration: GLSL TOP does NOT auto-declare; you must declare in source

On the GLSL TOP, assigning a uniform on the Vectors page populates the value but
does **not** add the declaration to the shader. Your shader source must contain
the explicit declaration:

```glsl
uniform float uLfo;       // required — GLSL TOP does not auto-declare
```

This differs from the GLSL POP (see Write_GLSL_POPs.md addendum). Confused
behaviour between TOP and POP is a common source of "shader compiles but uniform
reads as 0" bugs — they have opposite conventions.
