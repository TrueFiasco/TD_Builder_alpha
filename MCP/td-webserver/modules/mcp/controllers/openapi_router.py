...
class OpenAPIRouter:
	...
	def register_route(
		...
		"""
		Register a route dynamically (not from OpenAPI schema).

		Used for extensions like Network Editor MCP feedback endpoints.

		Args:
		    method: HTTP method (GET, POST, etc.)
		    path: URL path pattern (e.g., "/api/feedback/capture/top")
		    operation_id: Unique operation identifier
		    handler: Function to handle requests
		    has_request_body: Whether this route accepts a request body
		"""
		route = RouteDefinition(
			method=method.upper(),
			path_pattern=path,
		...
		self._routes_by_operation_id[operation_id] = route

		# Register the handler
		self._handlers[operation_id] = handler

		log_message(
			f"Registered dynamic route: {method} {path} -> {operation_id}",
			LogLevel.INFO
		)
