...
| **Multi-expert orchestration** | The `get_expert_prompt` prompts describe a plan → build → critic pipeline, but V0.1.1 has no automated orchestrator — you fetch and run each expert prompt individually. | **Deferred to V0.2.** Prompts now route all knowledge through the MCP tools; the cross-expert pipeline is manual for now. |
| **Self-improvement / curated expertise** | The experts' `self_improve` phase and the 18 curated `Agents/expertise/*.yaml` are not wired into the runtime (only `td_network_building.yaml` is read by code). | **Deferred to V0.2.** Those YAMLs plus three currently-unused engine modules (`engine/writers/lossless_writer.py`, `server_core/search/__init__.py`, `Agents/experts/__init__.py`) are kept for a future "connect in or remove" decision. |
