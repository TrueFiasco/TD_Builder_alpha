...
# ---------------------------------------------------------------------------
# `operator_types.json` is TRACKED and generated by kb_build/gen_operator_types.py
# from the committed census. An untracked byte-identical twin has historically
# lived in the main tree's corpus at
#   New KB build/Resources/operator_ground_truth/operator_types.json
# and CI read the tracked file (--gt-types) while every local default read the
# twin. They agreed only by being identical -- regenerate one and local runs and
# CI silently grade against different ground truth. W3 Census Lock makes the
# tracked file the source and leaves the corpus path as a legacy fallback only.
#
# NOTE the corpus directory is still authoritative for `params/` and
# `tox_expanded/` (live-TD captures, ~31 MB, genuinely not committable). Resolve
# the FILE from here; resolve those DIRECTORIES from the corpus. Never derive one
# from the other's parent.
GT_DIR: Path = REPO_ROOT / "eval" / "ground_truth"
GT_OPERATOR_TYPES: Path = GT_DIR / "operator_types.json"
GT_TD_CENSUS: Path = GT_DIR / "td_census.json"


def operator_types_path(legacy_fallback: Path | None = None) -> Path:
    """The tracked operator_types.json, or a legacy corpus copy if it is absent.

    The fallback exists so a checkout predating W3 (or a corpus-only tool) still
    resolves; it is never preferred over the tracked file.
    """
    if GT_OPERATOR_TYPES.exists():
        return GT_OPERATOR_TYPES
